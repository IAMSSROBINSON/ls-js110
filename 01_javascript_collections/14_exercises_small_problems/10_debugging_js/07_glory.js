...
On line 22 a `switch statement` is defined. The statement attempts to match an instance of a random number(result) between 1 and 20 with specific actions for matching numbers 1 and 20. 

The issue lies in the fact that if a case is matched then the statement does not `break` and instead follows through to the next case execution which again does not exit the statement but continues to follow through to the default statement which should only execute if neither case is matched. 

Placing a `break` statement on the line after each respective cases action should allow the matched case to execute the action and break out of the statement thereby allowing for no other case executions.

Now if `case 1` is matched it will invoke the respective action. If `case 20` is matched it will invoke the respective action and if any random number inbetween occurs then the `default` clause it executed.
