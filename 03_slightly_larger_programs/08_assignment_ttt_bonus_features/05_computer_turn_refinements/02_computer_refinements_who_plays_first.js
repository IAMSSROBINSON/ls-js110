...
const MESSAGES = require("./02_computer_refinements_who_plays_first.json");
const BOARD_SIZE = 9;
...
function userFirstSequence (board) {
  prompt(
...
function computerFirstSequence (board) {
...
function userChoosesWhoGoesFirst () {
  prompt(MESSAGES.promptWhoPlaysFirst);
...
function startProgram() {
  ...
  while (true) {
    ...
    let whoPlaysFirst = PLAYS_FIRST[randomFirstPlayer];
    ...
    while (true) {
      ...
      if (whoPlaysFirst === 'user') {
        userFirstSequence(board);
        if (isNoSquaresLeft(board) || isWinner(board)) break;
        computerFirstSequence(board);
        if (isNoSquaresLeft(board) || isWinner(board)) break;
      } else if (whoPlaysFirst === 'computer') {
        computerFirstSequence(board);
        if (isNoSquaresLeft(board) || isWinner(board)) break;
        userFirstSequence(board);
        if (isNoSquaresLeft(board) || isWinner(board)) break;
        ...
        whoPlaysFirst = userChoosesWhoGoesFirst();
      }
    }
